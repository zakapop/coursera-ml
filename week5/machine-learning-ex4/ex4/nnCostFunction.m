...
J = (1/m) * sum(sum((-y_i).*log(h)-(1-y_i).*log(1-h),2));

penalty_theta1 = Theta1(:, 2:end);
penalty_theta2 = Theta2(:, 2:end);
total_penalty = (lambda / (2*m)) * (sum(sum(penalty_theta1.^2)) + sum(sum(penalty_theta2.^2)));
